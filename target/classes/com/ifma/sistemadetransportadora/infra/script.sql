...
SELECT cd.nome as destino, round(ft.peso*10 + cd.taxa, 2) as valorCalculado 
FROM Cidade cd 
...
SELECT ft.codigo_frete , ft.descricao, ft.valor FROM Frete ft WHERE ft.valor = ( SELECT MAX(ft.valor) FROM Frete );

SELECT cd.nome, MAX(ft.codigo_cidade) AS QuantidadeFretes FROM Frete ft INNER JOIN Cidade cd ON cd.codigo_cidade = ft.codigo_cidade;
